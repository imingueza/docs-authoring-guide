...
Examples of commit messages could be: "(EN) Add section Conclusions" or "Fix some typos".
...
To minimize the probability of such an event we stablish the following rule:
...
These rules implie that when different Decidim Team members work on the same document, they need to coordinate and decide who edit which files.
