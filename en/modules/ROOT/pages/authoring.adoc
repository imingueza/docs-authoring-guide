...
Our collaboration workflow is based on Git.
...
There are some guidelines that are specific to our setup:
...
Now Visual Studio Code gives you the option of opening the new created folder.
You can open it, but it is better to follow the advice given in <<vscode-spell-checker>>.
...
So, directories `en/modules/ROOT/pages`, `ca/modules/ROOT/pages` and `es/modules/ROOT/pages` should look all the same, and all contain files named in English.
--
