...
The site is assembled from a bunch of individual documents, using a tool that can pull contents from a list of repositories and convert them to HTML.
...
All the things writers need to know are hopefully explained in section xref:authoring.adoc#authoring[Authoring documentation].
The tasks usually carried on by document administrators are described in section xref:administration.adoc#administration[Administering documentation].
