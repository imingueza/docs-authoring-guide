...
* An Antora configuration file [.path]#site.yml# that acts as a _playbook_ for the site (all document repositories are declared here, along with some global configuration options).
...
It contains all the presentation stuff (HTML templates, CSS files, Etc.)
...
[IMPORTANT]
--
In case of documents with one single `.adoc` file (i.e., the document is simple enough to not need splitting it up), we should:

* Remove file [.path]#single-page.adoc#.
* Remove the reference to [.path]#single-page.adoc# in file [.path]#nav.adoc#.
--
...
. Make a GitHub fork of the site repository.
